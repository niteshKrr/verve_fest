...
const Tug_of_war = () => {
  ...
  const handleSubmit = (e: any) => {
    ...
    axios
      .post(`http://127.0.0.1:8000/sports/tug_of_war`, {
        name,
        des,
        branch,
        roll,
      ...
      <Card className="sm:w-96">
        <CardHeader className="flex gap-4 my-3">
          ...
          <div className="flex flex-col">
            <p className="text-md font-bold">Register for Tug_of_war </p>
          </div>
        </CardHeader>
        <Divider />
        <CardBody>
          <div>
            <Input value={name} onChange={(e) => setName(e.target.value)} type="text" variant="underlined" label="name [captain name]" />
          </div>
          <div className="mt-4">
          <Textarea
          value={des}
